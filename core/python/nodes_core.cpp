...
NB_MODULE(nodes_core_py, m)
{
    ...
    nb::class_<entt::meta_any>(m, "meta_any")
        ...
        .def(
            "cast_int",
            [](const entt::meta_any& self) { return self.cast<int>(); })
        .def(
            "cast_float",
            [](const entt::meta_any& self) { return self.cast<float>(); })
        .def(
            "cast_double",
            [](const entt::meta_any& self) { return self.cast<double>(); })
        .def(
            "cast_bool",
            [](const entt::meta_any& self) { return self.cast<bool>(); })
        .def(
            "cast_string",
            [](const entt::meta_any& self) { return self.cast<std::string>(); })
        .def(
            "type_name",
            [](const entt::meta_any& self) {
                if (!self)
        ...
        .def("__repr__", [](const entt::meta_any& self) {
            if (!self)
                return std::string("meta_any(void)");
    ...
    m.def("to_meta_any", [](const nb::object& obj) -> entt::meta_any {
        ...
        // CRITICAL: Check bool BEFORE int, because in Python bool is a subclass
        // of int!
        if (nb::isinstance<nb::bool_>(obj)) {
            ...
            return entt::meta_any{ nb::cast<float>(
                obj) };  // Changed from double to float
        }
        ...
        .def(
            ...
            [](NodeSocket& s, const nb::object& obj) {
                ...
                if (nb::isinstance<nb::bool_>(obj)) {
                    ...
                    value = entt::meta_any{ nb::cast<int>(obj) };
                }
                else if (nb::isinstance<nb::float_>(obj)) {
                    value = entt::meta_any{ nb::cast<float>(obj) };
                }
                else if (nb::isinstance<nb::str>(obj)) {
                    value = entt::meta_any{ nb::cast<std::string>(obj) };
                }
                else {
                    throw std::runtime_error(
                        "Unsupported type for socket default value");
                }
                s.dataField.value = value;
            },
            nb::arg("value"),
            ...
            [](Node& n,
               const std::vector<std::pair<std::string, bool>>& requests) {
                ...
                for (const auto& [identifier, is_input] : requests) {
                    result.push_back(
                        is_input ? n.get_input_socket(identifier.c_str())
    ...
    nb::class_<NodeTree>(m, "NodeTree")
        ...
        .def(
            "add_nodes_batch",
            [](NodeTree& tree, const std::vector<std::string>& node_types) {
                std::vector<Node*> result;
            ...
            [](NodeTree& tree,
               ...
               bool refresh_topology) {
                ...
                for (size_t i = 0; i < links.size(); ++i) {
                    bool should_refresh =
                        (i == links.size() - 1) && refresh_topology;
                    auto link = tree.add_link(
                        links[i].first, links[i].second, false, should_refresh);
