...
TEST_F(NodeExecTest, CacheTest)
{
    ...
    tree->add_link(
        node0->get_output_socket("result"), node1->get_input_socket("a"));
    tree->add_link(
        node1->get_output_socket("result"), node2->get_input_socket("a"));
    ...
    ASSERT_EQ(
        result.cast<int>(),
        14);  // node0: 1+2=3, node1: 3+10=13, node2: 13+1=14
}
...
TEST_F(NodeExecTest, LinkChangeOnlyAffectsDownstream)
{
    ...
    std::cout << "  node0 dirty: " << executor->is_node_dirty(node0)
              << std::endl;
    std::cout << "  node1 dirty: " << executor->is_node_dirty(node1)
              << std::endl;
    std::cout << "  node2 dirty: " << executor->is_node_dirty(node2)
              << std::endl;
    ...
    // node0 should be clean (upstream), node1 and node2 should be dirty
    // (downstream)
...
TEST_F(NodeExecTest, ReconnectUsesNewUpstreamData)
{
    ...
    tree->add_link(
        up1->get_output_socket("result"), up2->get_input_socket("a"));
    tree->add_link(
        up2->get_output_socket("result"), up3->get_input_socket("a"));
    ...
    tree->add_link(
        down1->get_output_socket("result"), down2->get_input_socket("a"));
    tree->add_link(
        down2->get_output_socket("result"), down3->get_input_socket("a"));
    ...
    tree->add_link(
        B->get_output_socket("result"), down1->get_input_socket("a"));

    std::cout << "\n=== Phase 1: Initial Execution (up3->A->B->down1) ==="
              << std::endl;
    executor->prepare_tree(tree.get());
    ...
    // Check dirty state: upstream should be clean, B and downstream should be
    // dirty
    std::cout << "Dirty states after disconnect:" << std::endl;
    std::cout << "  up1 (should be clean): " << executor->is_node_dirty(up1)
              << std::endl;
    std::cout << "  up2 (should be clean): " << executor->is_node_dirty(up2)
              << std::endl;
    std::cout << "  up3 (should be clean): " << executor->is_node_dirty(up3)
              << std::endl;
    std::cout << "  A (should be clean): " << executor->is_node_dirty(A)
              << std::endl;
    std::cout << "  B (should be dirty): " << executor->is_node_dirty(B)
              << std::endl;
    std::cout << "  down1 (should be dirty): " << executor->is_node_dirty(down1)
              << std::endl;
    std::cout << "  down2 (should be dirty): " << executor->is_node_dirty(down2)
              << std::endl;
    std::cout << "  down3 (should be dirty): " << executor->is_node_dirty(down3)
              << std::endl;
    ...
    std::cout << "up3 result (should be cached 62): " << up3_result
              << std::endl;
    ASSERT_EQ(up3_result, 62);
    ...
    std::cout << "B result (CRITICAL: should be 63, not 163): " << B_result
              << std::endl;
    ASSERT_EQ(B_result, 63)
        << "B should use up3's cached value (62), not A's old value (162)";

    // down chain should be 64, 65, 66 (B's 63 + 1, + 1, + 1)
    executor->sync_node_to_external_storage(
        down1->get_output_socket("result"), result);
    std::cout << "down1 result (should be 64): " << result.cast<int>()
              << std::endl;
    ASSERT_EQ(result.cast<int>(), 64);

    executor->sync_node_to_external_storage(
        down2->get_output_socket("result"), result);
    std::cout << "down2 result (should be 65): " << result.cast<int>()
              << std::endl;
