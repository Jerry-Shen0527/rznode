   ...
   protected:
    void SetUp() override
    {
        ...
        merge_node.set_execution_function([](ExeParams params) {
            ...
            int sum = 0;
            std::cout << "[Merge Node] Executing with " << inputs.size()
                      << " inputs: ";
        ...
        producer_node.set_execution_function([](ExeParams params) {
            ...
            std::cout << "[Producer Node " << params.node_.ID.Get()
                      << "] Producing value: " << val << std::endl;
...
// This test simulates: A->C, delete link, then node C still gets A's OLD cached
// value
TEST_F(SocketGroupBugsTest, DeleteLinkShouldInvalidateCache)
{
    ...
    executor->sync_node_from_external_storage(
        nodeA->get_input_socket("value"), 100);
    executor->sync_node_from_external_storage(
        nodeB->get_input_socket("value"), 200);
    executor->execute_tree(tree.get());

    entt::meta_any result;
    executor->sync_node_to_external_storage(
        nodeC->get_output_socket("result"), result);
    std::cout << "Result: " << result.cast<int>() << " (expected: 300)"
              << std::endl;
    ...
    std::cout << "socketC_in1 cached value: "
              << (cached_val_in1 && *cached_val_in1
                      ? cached_val_in1->cast<int>()
                      : -999)
              << std::endl;
    std::cout << "socketC_in2 cached value: "
              << (cached_val_in2 && *cached_val_in2
                      ? cached_val_in2->cast<int>()
                      : -999)
              << std::endl;
    ...
    std::cout << "  nodeA: " << executor->is_node_dirty(nodeA)
              << " (should be false)" << std::endl;
    std::cout << "  nodeB: " << executor->is_node_dirty(nodeB)
              << " (should be false)" << std::endl;
    std::cout << "  nodeC: " << executor->is_node_dirty(nodeC)
    ...
    // CRITICAL CHECK: After deleting link, the socket's cached value should be
    // marked invalid But the socket pointer is now dangling! The socket was
    // deleted!
    std::cout << "\n--- Check if deleted socket's cache was invalidated ---"
              << std::endl;
    std::cout << "Note: socketC_in1 was deleted, so we can't check it directly"
              << std::endl;

    std::cout << "\n--- Phase 3: Re-execute after link deletion ---"
              << std::endl;
    executor->prepare_tree(tree.get());
    executor->execute_tree(tree.get());
    ...
    executor->sync_node_to_external_storage(
        nodeC->get_output_socket("result"), result);
    std::cout << "Result: " << result.cast<int>()
              << " (expected: 200, CRITICAL BUG if 300)" << std::endl;

    // BUG: Currently returns 300 (cached A+B), should return 200 (only B)
    ASSERT_EQ(result.cast<int>(), 200)
        << "CRITICAL BUG: Merge node should only receive B's value (200), not "
           "cached A+B (300)";
}

// Bug 1b: Reconnect link - downstream executes but gets OLD cached value
// This is the REAL bug: A->C, disconnect, D->C, C executes but gets A's old
// value!
TEST_F(SocketGroupBugsTest, ReconnectLinkGetsOldCachedValue)
{
    std::cout << "\n=== TEST: ReconnectLinkGetsOldCachedValue ===" << std::endl;
    std::cout << "This simulates: A->C (result=100), disconnect, D->C (should "
                 "be 500, bug if 100)"
              << std::endl;
    ...
    auto nodeC = tree->add_node("merge");
    auto nodeDown =
        tree->add_node("producer");  // Downstream node to observe changes
    ...
    executor->sync_node_to_external_storage(
        nodeC->get_output_socket("result"), result);
    std::cout << "C result: " << result.cast<int>() << " (expected: 100)"
              << std::endl;
    ASSERT_EQ(result.cast<int>(), 100);

    executor->sync_node_to_external_storage(
        nodeDown->get_output_socket("result"), result);
    std::cout << "Downstream result: " << result.cast<int>()
              << " (expected: 100)" << std::endl;
    ASSERT_EQ(result.cast<int>(), 100);

    std::cout << "\n--- Phase 2: Disconnect A->C, Connect D->C ---"
              << std::endl;
    ...
    std::cout << "\n--- Phase 3: Execute with D->C (D produces 500) ---"
              << std::endl;
    executor->prepare_tree(tree.get());
    executor->sync_node_from_external_storage(
        nodeD->get_input_socket("value"), 500);
    executor->execute_tree(tree.get());

    executor->sync_node_to_external_storage(
        nodeC->get_output_socket("result"), result);
    std::cout << "C result: " << result.cast<int>()
              << " (expected: 500, CRITICAL BUG if 100)" << std::endl;

    executor->sync_node_to_external_storage(
        nodeDown->get_output_socket("result"), result);
    std::cout << "Downstream result FIRST execution: " << result.cast<int>()
              << " (might still be 100 if cached)" << std::endl;

    std::cout << "\n--- Phase 4: Execute again (downstream should now see "
                 "updated value) ---"
              << std::endl;
    std::cout << "Downstream node dirty state: "
              << executor->is_node_dirty(nodeDown)
              << " (should be true if fix works)" << std::endl;
    executor->prepare_tree(tree.get());
    executor->execute_tree(tree.get());

    executor->sync_node_to_external_storage(
        nodeDown->get_output_socket("result"), result);
    std::cout << "Downstream result SECOND execution: " << result.cast<int>()
              << " (expected: 500, CRITICAL BUG if 100)" << std::endl;
    std::cout
        << "If downstream executed but got 100 instead of 500, that's the bug!"
        << std::endl;

    ASSERT_EQ(result.cast<int>(), 500)
        << "CRITICAL BUG: Downstream should get new value (500), not old "
           "cached value (100)!";
}
...
TEST_F(SocketGroupBugsTest, PersistentCacheTypeMismatch)
{
    ...
    std::cout << "Simulates: int socket gets cached, deleted, new float socket "
                 "created at same position"
              << std::endl;
    ...
    float_producer.set_execution_function([](ExeParams params) {
        ...
        std::cout << "[Float Producer " << params.node_.ID.Get()
                  << "] Producing: " << val << std::endl;
        params.set_output("result", val);
        return true;
    });
    ...
    executor->prepare_tree(tree.get());
    executor->sync_node_from_external_storage(
        nodeA->get_input_socket("value"), 42);
    executor->execute_tree(tree.get());

    entt::meta_any result;
    executor->sync_node_to_external_storage(
        nodeC->get_output_socket("result"), result);
    std::cout << "Result: " << result.cast<int>() << " (type: int)"
              << std::endl;
    ...
    std::cout
        << "\n--- Phase 2: Delete int producer, socket should be removed ---"
        << std::endl;
    ...
    for (auto* input : nodeC->get_inputs()) {
        if (input->socket_group && !input->is_placeholder()) {
            remaining++;
            std::cout << "Remaining socket: " << input->identifier
                      << " (type: " << input->type_info.info().name() << ")"
                      << std::endl;
        }
    }
    ASSERT_EQ(remaining, 0) << "All sockets should be removed";

    std::cout << "\n--- Phase 3: Create new socket at 'same' position with int "
                 "type ---"
              << std::endl;
    auto nodeD = tree->add_node("producer");  // int producer
    auto socketD_out = nodeD->get_output_socket("result");

    // Create NEW socket with SAME identifier "input_0" but it's a different
    // socket object
    auto socketC_in_new = nodeC->group_add_socket(
        "inputs",
        type_name<int>().c_str(),
    ...
    std::cout << "Are they different objects? "
              << (socketC_in1 != socketC_in_new ? "YES" : "NO") << std::endl;

    tree->add_link(socketD_out, socketC_in_new);

    std::cout << "\n--- Phase 4: Execute with new int producer ---"
              << std::endl;
    executor->prepare_tree(tree.get());
    executor->sync_node_from_external_storage(
        nodeD->get_input_socket("value"), 999);
    executor->execute_tree(tree.get());
    ...
    ASSERT_TRUE(nodeC->execution_failed.empty())
        << "Same type should work: " << nodeC->execution_failed;
...
{
    std::cout
        << "\n=== TEST: DeleteNodesShouldRemoveSocketsAndAllowReconnection ==="
        << std::endl;
    ...
    // When deleting nodes, their sockets should be removed, and connected group
    // sockets in C should also be removed
    tree->delete_node(nodeA);
    ...
    std::cout << "Remaining sockets in nodeC's input group after deletion:"
              << std::endl;
    int remaining_sockets = 0;
    for (auto* input : nodeC->get_inputs()) {
        if (input->socket_group &&
            input->socket_group->identifier == "inputs") {
            if (!input->is_placeholder()) {
                std::cout << "  - " << input->identifier
                          << " (UI: " << input->ui_name << ")" << std::endl;
                remaining_sockets++;
            }
        }
    }
    std::cout << "Total non-placeholder sockets: " << remaining_sockets
              << " (should be 0)" << std::endl;

    std::cout << "\n--- Phase 3: Create new node D and connect to C ---"
              << std::endl;
    auto nodeD = tree->add_node("producer");
    ...
    std::cout << "Created new socket in C: " << socketC_in3->identifier
              << std::endl;
    std::cout << "Socket type: " << socketC_in3->type_info.info().name()
              << std::endl;
    ...
    executor->sync_node_to_external_storage(
        nodeC->get_output_socket("result"), result);
    std::cout << "Result: " << result.cast<int>() << " (expected: 500)"
              << std::endl;
    ...
    ASSERT_TRUE(nodeC->execution_failed.empty())
        << "CRITICAL BUG: Got error: " << nodeC->execution_failed;
...
{
    std::cout
        << "\n=== TEST: DeleteMiddleLinkShouldOnlyAffectThatConnection ==="
        << std::endl;
    ...
    executor->sync_node_from_external_storage(
        nodeD->get_input_socket("value"), 30);
    executor->execute_tree(tree.get());

    entt::meta_any result;
    executor->sync_node_to_external_storage(
        nodeC->get_output_socket("result"), result);
    std::cout << "Result: " << result.cast<int>() << " (expected: 60)"
              << std::endl;
    ...
    std::cout << "\n--- Phase 3: Re-execute (should be A+D only) ---"
              << std::endl;
    executor->prepare_tree(tree.get());
    executor->execute_tree(tree.get());
    ...
    executor->sync_node_to_external_storage(
        nodeC->get_output_socket("result"), result);
    std::cout << "Result: " << result.cast<int>()
              << " (expected: 40, CRITICAL BUG if 60)" << std::endl;

    ASSERT_EQ(result.cast<int>(), 40)
        << "Should be A+D (10+30=40), not A+B+D (60)";
}
...
{
    std::cout << "\n=== TEST: SocketGroupCacheMustBeInvalidatedOnLinkDelete ==="
              << std::endl;
    ...
    std::cout << "\n--- Phase 3: Check if socket cache was invalidated ---"
              << std::endl;
    // After deletion and notification, the socket's cached value should be
    // marked invalid This test checks the internal state

    std::cout << "Node C dirty state: " << executor->is_node_dirty(nodeC)
              << " (should be true)" << std::endl;
    ASSERT_TRUE(executor->is_node_dirty(nodeC))
        << "Node C should be marked dirty after link deletion";
    ...
    executor->sync_node_to_external_storage(
        nodeC->get_output_socket("result"), result);
    std::cout << "Result: " << result.cast<int>()
              << " (expected: 0, bug if 100)" << std::endl;

    ASSERT_EQ(result.cast<int>(), 0)
        << "Merge with no inputs should return 0, not cached 100";
}

// Bug 2: Type mismatch error when reconnecting after intermediate node
// Simplified scenario: A.sock1->C, disconnect, D.sock->C (different int value),
// disconnect, A.sock1->C The issue is persistent cache holds wrong type/value
TEST_F(SocketGroupBugsTest, TypeMismatchAfterIntermediateNodeWithDifferentType)
{
    std::cout
        << "\n=== TEST: TypeMismatchAfterIntermediateNodeWithDifferentType ==="
        << std::endl;
    ...
    // Create second socket in C and connect (simulating A has 2 outputs by
    // using same output)
    auto socketC_in2 = nodeC->group_add_socket(
        "inputs",
        type_name<int>().c_str(),
    ...
    executor->sync_node_to_external_storage(
        nodeC->get_output_socket("result"), result1);
    std::cout << "C result with A's two connections: " << result1.cast<int>()
              << std::endl;
    ASSERT_EQ(result1.cast<int>(), 200);  // 100 + 100

    // Step 2: Disconnect both links
    std::cout << "\n--- Step 2: Disconnect A.sock1->C and A.sock2->C ---"
              << std::endl;
    std::cout << "Before deletion, socketC_in1 ptr: " << (void*)socketC_in1
              << std::endl;
    std::cout << "Before deletion, socketC_in2 ptr: " << (void*)socketC_in2
              << std::endl;
    tree->delete_link(link1);
    tree->delete_link(link2);
    executor->notify_node_dirty(nodeC);
    ...
    std::cout << "NodeC has " << remaining_inputs.size()
              << " input sockets remaining" << std::endl;
    for (auto* sock : remaining_inputs) {
        std::cout << "  Socket: " << (void*)sock
                  << " isPlaceholder=" << sock->is_placeholder() << std::endl;
    }

    // Step 3: Create D and connect to C - create NEW socket
    std::cout << "\n--- Step 3: Create D, D.sock->C (creating new socket) ---"
              << std::endl;
    auto nodeD = tree->add_node("producer");
    auto socketD_out = nodeD->get_output_socket("result");
    ...
    auto socketC_in_new = nodeC->group_add_socket(
        ...
        "input_new",
        PinKind::Input);
    std::cout << "Created new socket socketC_in_new ptr: "
              << (void*)socketC_in_new << std::endl;
    ...
    std::cout << "Node C execution_failed after D connection: '"
              << nodeC->execution_failed << "'" << std::endl;

    entt::meta_any result2;
    executor->sync_node_to_external_storage(
        nodeC->get_output_socket("result"), result2);
    std::cout << "C result with D connection - has value: "
              << (result2 ? "yes" : "no") << std::endl;
    if (result2) {
        std::cout << "C result with D connection: " << result2.cast<int>()
                  << std::endl;
    ...
    std::cout << "\n--- Step 5: Reconnect A.sock1->C (via new socket) ---"
              << std::endl;
    ...
    try {
        ...
        auto socketC_in_reconnect = nodeC->group_add_socket(
            ...
            "input_reconnect",
            PinKind::Input);
        std::cout << "Created socketC_in_reconnect ptr: "
                  << (void*)socketC_in_reconnect << std::endl;
        ...
        executor->sync_node_from_external_storage(
            nodeA->get_input_socket("value"), 500);
    ...
    std::cout << "Node C execution_failed: '" << nodeC->execution_failed
              << "' (should be empty)" << std::endl;

    // BUG CHECK: execution_failed should be empty
    ASSERT_TRUE(nodeC->execution_failed.empty())
        << "Node C should not have execution error, but got: "
        << nodeC->execution_failed;

    entt::meta_any result_final;
    executor->sync_node_to_external_storage(
        nodeC->get_output_socket("result"), result_final);
    std::cout << "C final result: " << result_final.cast<int>()
