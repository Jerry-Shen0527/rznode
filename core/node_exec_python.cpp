...
std::string PythonCodeGenerator::generate(const NodeTree* tree, Node* required_node)
{
    if (!tree) {
...
void PythonCodeGenerator::collect_required_nodes(const NodeTree* tree, Node* required_node)
{
    if (required_node) {
...
void PythonCodeGenerator::generate_node_creation(const NodeTree* tree)
{
    ...
    for (Node* node : execution_order_) {
        ...
        std::string node_type = node->typeinfo ? node->typeinfo->id_name : "unknown";
        std::string ui_name = node->ui_name.empty() ? var_name : node->ui_name;
        
        std::ostringstream line;
        line << var_name << " = g.createNode(\"" << node_type << "\", name=\"" << ui_name << "\")";
...
void PythonCodeGenerator::generate_connections(const NodeTree* tree)
{
    ...
    for (const auto& link : tree->links) {
        ...
        if (!from_socket || !to_socket) continue;
...
void PythonCodeGenerator::generate_input_assignments(const NodeTree* tree)
{
    ...
    for (Node* node : execution_order_) {
        ...
        for (NodeSocket* socket : input_sockets) {
            // Skip sockets that are connected (they get values from other nodes)
            ...
            std::string var_name = get_node_variable_name(node);
            std::ostringstream line;
            line << "# (" << var_name << ", \"" << socket->identifier << "\"): value,";
            write_line(line.str());
            has_inputs = true;
        }
    }
    ...
    for (Node* node : execution_order_) {
        ...
        for (NodeSocket* socket : output_sockets) {
            ...
            if (is_terminal) {
                std::string var_name = get_node_variable_name(node);
                std::ostringstream line;
                line << "g.markOutput(" << var_name << ", \"" << socket->identifier << "\")";
                write_line(line.str());
                has_outputs = true;
            }
        }
...
void PythonCodeGenerator::generate_output_retrieval(const NodeTree* tree, Node* required_node)
{
    if (options_.include_comments) {
    ...
    if (required_node) {
        ...
        for (NodeSocket* socket : outputs) {
            std::string result_var = "result_" + sanitize_identifier(socket->identifier);
            std::ostringstream line;
            line << result_var << " = g.getOutput(" << var_name << ", \"" << socket->identifier << "\")";
            write_line(line.str());
            
            if (options_.include_comments) {
                line.str("");
                line << "print(f\"" << socket->identifier << " = {" << result_var << "}\")";
                write_line(line.str());
            }
        }
    } else {
        ...
        for (Node* node : execution_order_) {
            ...
            for (NodeSocket* socket : outputs) {
                bool is_terminal = !tree->is_pin_linked(socket);
                if (is_terminal) {
                    std::string result_var = var_name + "_" + sanitize_identifier(socket->identifier);
                    std::ostringstream line;
                    line << result_var << " = g.getOutput(" << var_name << ", \"" << socket->identifier << "\")";
                    write_line(line.str());
                    ...
                    if (options_.include_comments) {
                        line.str("");
                        line << "print(f\"" << var_name << "." << socket->identifier << " = {" << result_var << "}\")";
                        write_line(line.str());
                    }
                }
            }
...
void PythonCodeGenerator::topological_sort(const NodeTree* tree, Node* start_node)
{
    // This is handled by determine_execution_order using tree's built-in toposort
}

void PythonCodeGenerator::visit_node_dfs(Node* node, std::set<Node*>& visited, std::set<Node*>& recursion_stack)
{
